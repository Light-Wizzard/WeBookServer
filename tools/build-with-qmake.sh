...
#
#
wget -c -nv https://github.com/Skycoder42/QtService/releases/download/2.0.2-3/qtservice-gcc_64-5.14.1.tar.xz;
mkdir -p QtService;        
7z x "qtservice-gcc_64-5.14.1.tar.xz" -so | 7z x -aoa -si -ttar -o"./QtService";
#
# It is worth mentioning that rsync gives different treatment to the source directories with a trailing slash /. 
# If you add a trailing slash on the source directory it will copy only the contents of the directory to the destination directory. 
# When the trailing slash is omitted rsync will copy the source directory inside the destination directory.
# 
# /usr/include/qt/QtService/
sudo rsync -ar ./QtService/gcc_64/include/ "/usr/include/qt";
sudo chown -R "${USER}:${USER}" "/usr/include/qt";

echo "Test: Include Directory:";
if [ -f "/usr/include/qt/QtService/service.h" ]; then
    echo "service.h found in the correct location: /usr/include/qt/QtService/service.h";
else
    echo "Error: service.h Not found in the correct location: /usr/include/qt/QtService/service.h";
    ls "/usr/include/qt/";
    echo "Include: /usr/include/qt/QtService/"
    ls "/usr/include/qt/QtService/";
    # gcc_64
fi
# /usr/lib
sudo rsync -ar ./QtService/gcc_64/lib/ "/usr/lib";
#sudo chown -R "${USER}:${USER}" "/usr/lib";

echo "Library Directory";
if [ -f "/usr/lib/libQt5Service.so" ]; then
    echo "libQt5Service.so found in the correct location: /usr/lib/libQt5Service.so";
else
    echo "Error: libQt5Service.so Not found in the correct location: /usr/lib";
    ls "/usr/lib";
    #ls -LR /usr;
fi
