...
QByteArray QAESEncryption::getPadding(int currSize, int alignment)
{
    ...
    switch(m_padding)
    {
        ...
        case Padding::PKCS7:
            ...
            break;
        case Padding::ISO:
            if (size > 0) return QByteArray (size - 1, 0x00).prepend(0x80);
            break;
        default:
            return QByteArray(size, 0x00);
            break;
...
{
    if (m_mode >= CBC && (iv.isNull() || iv.size() != m_blocklen)) return QByteArray();
    ...
    {
        case ECB:
            for(int i=0; i < alignedText.size(); i+= m_blocklen) ret.append(cipher(expandedKey, alignedText.mid(i, m_blocklen)));
            break;
        case CBC:
        {
        ...
        case CFB:
        {
            ...
            for(int i=0; i < alignedText.size(); i+= m_blocklen)
            {
                if (i+m_blocklen < alignedText.size())
                    ret.append(byteXor(alignedText.mid(i+m_blocklen, m_blocklen), cipher(expandedKey, ret.mid(i, m_blocklen))));
            }
        }
            break;
        case OFB:
    ...
    switch(m_mode)
    {
        ...
        case CFB:
        {
            ...
            for(int i=0; i < rawText.size(); i+= m_blocklen)
            {
                ...
                {
                    ret.append(byteXor(rawText.mid(i+m_blocklen, m_blocklen), cipher(expandedKey, rawText.mid(i, m_blocklen))));
                }
            }
        }
            break;
